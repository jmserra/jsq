...
| `o` | insert a blank row — opens a generated `INSERT` skeleton in `$EDITOR` (auto-generated columns omitted, PK/UNIQUE flagged); `:wq` runs it. Also works from the **user list**, where the new record is a user: it opens your engine's `CREATE USER` + `GRANT` template with the placeholder name selected, and runs exactly what you save. The starting grant is deliberately narrow (`SELECT` on the current database) — widen it by hand. When it lands, the user list reloads |
