...
| `V` | visual row-select mode — highlights the current row, then `j`/`k` (and `g`/`G`) extend the selection, `o` swaps the moving edge to the other end (vim-style), `y` yanks the selected rows as a JSON array of objects (reports `N rows copied to clipboard`) and exits, `Esc` cancels |
...
| `o` | insert a blank row — opens a generated `INSERT` skeleton in `$EDITOR` (auto-generated columns omitted, PK/UNIQUE flagged); `:wq` runs it. Also works on the **user list**, where the new record is a user (your engine's `CREATE USER` + `GRANT` template, placeholder name selected), and on a **user's privileges**, where it is one more grant for that user. Both run exactly what you save; the starting grant is deliberately narrow (`SELECT` on the current database) — widen it by hand |
| `D` | delete the current row — opens the generated PK-keyed `DELETE` in `$EDITOR`; `:wq` confirms, `:q!` aborts. It deletes the record of whatever list you're on: on the **user list** it drops the user (Postgres carries the `REASSIGN OWNED`/`DROP OWNED` remedies as comments, since a role that owns objects can't just be dropped), and on a **user's privileges** it revokes — the row under the cursor *is* the grant, so the `REVOKE` is built straight from it (a Postgres role attribute becomes `ALTER ROLE … NO…`, a role membership a `REVOKE … FROM …`) |
...
| `u` | go to the user list — every account on the server (MySQL) or role in the cluster (Postgres). `Enter` shows that user's privileges in the grid: one row per grant, normalised to `scope · object · privilege · grantable`, widest scope first — `global`/`attribute`, then the roles they belong to, then per-database, per-schema, per-table and per-column grants. `o` grants more and `D` revokes the row under the cursor; on the list itself `o` creates a user and `D` drops one — all through `$EDITOR` (SQLite has no users, so it reports that instead) |
...
with the caret on the last character; `←`/`→` move it, `Home`/`End` (or
...
- **Splits are panes, not tabs.** `Space v` duplicates the current view into a
  second pane on the right, `Space s` stacks one below; each pane then navigates
  independently — its own
  ...
  room when you aren't using it. The layout is **columns of stacked panes**:
  `Space v` opens a new column, `Space s` adds to the current one. (This is where
  it parts company with vim: a `Space v` *after* a `Space s` opens a full-height
  column rather than splitting only the focused pane's half.)
