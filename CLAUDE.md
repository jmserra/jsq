...
the same query with the same args, without re-probing. Read-only for now — the
`CREATE USER`/`GRANT` path would go through the `$EDITOR` full path (a seed in
`sqlgen.go` → `execRawCmd` → `askMutation` when safe), like `o`.
