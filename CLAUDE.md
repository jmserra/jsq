...
**Users** (`u`, from the table list or the grid): `Engine.Users()` fills a fourth
`sidebar` (`a.users` + the parallel `a.userList`, which `findUser` maps a label
back through — the sidebar carries names, not structs; MySQL's label is
`user@host`, since the host is half the account's identity). Enter runs
`grantsCmd`, which asks the engine for the query (`GrantsSQL(ctx, user)` — it
takes a ctx because MySQL probes whether `mysql.role_edges` is readable before
composing, so 5.7 and unprivileged connections drop that part instead of failing)
and runs it as an ordinary adHoc read. Every engine normalises to the same four
columns — `scope · object · privilege · grantable` — so one grid shape covers
both dialects; sqlite has no users (empty list → status, empty SQL → error). The
grants read is jsq-authored, so its values are **bound** (invariant 7), which is
...
`-- conn · db` line, lands the cursor on the `newuser` placeholder and selects it
(`selectWord`, `viw` — the name sits inside `'quotes'` on MySQL and `"quotes"` on
pg, so word-wise selection covers both). From there it is exactly the o/D/p path:
runs verbatim on `:wq`, `askMutation` first when `a.safe`, errView on failure.
The one addition is `editorSeed.users`, which rides seed →`editorSubmitMsg` →
...
- **MySQL**: URL → driver DSN via `mysql.Config` in `mysqlDSN` (don't hand-format).
  `MultiStatements` is **on**: without it the driver rejects `CREATE USER …;
  GRANT …;` (and any other multi-statement `s` submission) as a syntax error,
  while Postgres has always accepted them — verified both ways against 9.7.1. It
  only affects statements sent whole (Exec/Query with no args); everything jsq
  parameterizes is single-statement and still prepares normally.
